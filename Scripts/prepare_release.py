...
class Release:
    ...
    VERNUM_RE = re.compile("^(?P<num>[0-9]+[.][0-9]+[.][0-9]+)(?P<suffix>-.+)?$")
    ...
    VERSION_COMMENT = Template("Version $version, year 2018+$year_delta, month $month, day $day")
    VERSION_VALUE = Template("$version_prefix.$version_date$version_suffix")
    ...
    def update_build_props_file(self) -> None:
        ...
        for version_element in xml.iter("VersionPrefix"):
            ...
            version_element.text = self.VERSION_VALUE.substitute(template_values)
            comment = ElementTree.Comment(self.VERSION_COMMENT.substitute(template_values))
    ...
    def prepare(self):
        ...
        # TODO: Enable once we move to newsfragments
